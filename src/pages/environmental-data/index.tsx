...
import { getEnvironmentalData, deleteEnvironmentalDataById } from 'apiSdk/environmental-data';
import { EnvironmentalDataInterface } from 'interfaces/environmental-data';
import { Error } from 'components/error';
import { AccessOperationEnum, AccessServiceEnum, useAuthorizationApi, withAuthorization } from '@roq/nextjs';

function EnvironmentalDataListPage() {
  ...
  return (
    <AppLayout>
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('environmental_data', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Link href={`/environmental-data/create`}>
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  ...
                  {hasAccess('environmental_data', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                    <Th>Edit</Th>
                  )}
                  {hasAccess('environmental_data', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>View</Th>
                  )}
                  {hasAccess('environmental_data', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                    <Th>Delete</Th>
                  )}
                </Tr>
              </Thead>
              <Tbody>
                {data?.map((record) => (
                  <Tr key={record.id}>
                    ...
                    {hasAccess('environmental_data', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <NextLink href={`/environmental-data/edit/${record.id}`} passHref legacyBehavior>
                    ...
                    {hasAccess('environmental_data', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <NextLink href={`/environmental-data/view/${record.id}`} passHref legacyBehavior>
