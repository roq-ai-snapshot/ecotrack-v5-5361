  ...
  const { data, error, isLoading, mutate } = useSWR<BusinessOrganizationInterface[]>(
    ...
    () =>
      getBusinessOrganizations({
        relations: ['user', 'action_plan.count', 'environmental_data.count', 'environmental_goal.count'],
      }),
  );
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr key={record.id}>
                    ...
                    {hasAccess('action_plan', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.action_plan}</Td>
                    )}
                    {hasAccess('environmental_data', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.environmental_data}</Td>
                    )}
                    {hasAccess('environmental_goal', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.environmental_goal}</Td>
                    )}
                    {hasAccess('business_organization', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                      <Td>
